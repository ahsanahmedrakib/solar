import { TrendingUp, Sun, Zap, Users, BarChart3 } from "lucide-react";

const kpis = [
  { label: "Conversion Rate", value: "3.84%", change: "+0.6%", up: true, color: "#f59e0b" },
  { label: "Avg Order Value", value: "$1,932", change: "+$124", up: true, color: "#10b981" },
  { label: "Return Rate", value: "2.1%", change: "-0.3%", up: true, color: "#3b82f6" },
  { label: "Customer LTV", value: "$8,240", change: "+$420", up: true, color: "#8b5cf6" },
];
        ...
        <div className="admin-section-card">
          ...
          <div className="admin-section-body">
            <div style={{ display: "flex", gap: 20, marginBottom: 16 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "var(--admin-text-secondary)" }}>
                ...
                return (
                  <div key={d.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 8, height: "100%", justifyContent: "flex-end" }}>
                    <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 140 }}>
                      <div style={{ width: "45%", height: revH, background: "#f59e0b", borderRadius: "3px 3px 0 0", opacity: 0.85 }} />
                      <div style={{ width: "45%", height: ordH, background: "#3b82f6", borderRadius: "3px 3px 0 0", opacity: 0.85 }} />
                    </div>
                    <span style={{ fontSize: 10, color: "var(--admin-text-muted)" }}>{d.month}</span>
                  </div>
                );
              })}
            </div>
            ...
            {[
              ...
              <div key={s.source} style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                  <span style={{ fontSize: 13, color: "var(--admin-text-secondary)" }}>{s.source}</span>
                  <span style={{ fontSize: 13, fontWeight: 700, color: "var(--admin-text-primary)" }}>{s.pct}%</span>
                </div>
                <div style={{ height: 6, background: "var(--admin-surface-2)", borderRadius: 10, overflow: "hidden" }}>
                  <div
                    style={{
                      height: "100%",
                      width: `${s.pct}%`,
          ...
          <tbody>
            {topProducts.map((p, i) => (
              <tr key={p.name}>
                ...
                <td style={{ color: "var(--admin-text-secondary)" }}>{p.units}</td>
                <td>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ flex: 1, height: 6, background: "var(--admin-surface-2)", borderRadius: 10, overflow: "hidden" }}>
                      <div style={{ height: "100%", width: `${p.pct}%`, background: "var(--admin-accent)", borderRadius: 10 }} />
                    </div>
                    <span style={{ fontSize: 11, color: "var(--admin-text-muted)", width: 30 }}>{p.pct}%</span>
                  </div>
                </td>
              </tr>
            ))}
