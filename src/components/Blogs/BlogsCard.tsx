...
const BlogsCard = ({ blogs }: { blogs: Blog[] }) => {
  return (
      ...
      {blogs?.map((blog, index) => (
        ...
        >
          ...
          >
            ...
            {/* Content box */}
            <div className="relative z-10 w-full rounded-lg p-5 backdrop-blur-md transition-all duration-300 border bg-gold-900/40 backdrop-brightness-90 border-white/20 group-hover:bg-gold-900/70 group-hover:border-accent-500/30">
              {/* Optional category badge */}
              {blog.category && (
                <span className="inline-block mb-2 text-[10px] font-semibold uppercase tracking-wider text-accent-400">
                  {blog.category}
                </span>
              )}
              ...
              {/* Description – appears on hover (if available) */}
              {blog.content && (
